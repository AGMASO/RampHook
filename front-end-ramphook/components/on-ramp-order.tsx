...
import onRampOrderMain from "@/lib/scripts/onRampOrderMain";
...
export default function OnRampOrder({
  ...
  const handleOnRampMain = async () => {
    ...
    try {
      //! MVP: Only DAI onRamp order is implemented
      if (
        chainId == 8453 &&
        selectedToken == "0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb"
      ) {
        console.log("Executing onRampOrder for DAI on Base Mainnet");

        await onRampOrderMain({
          amountToSell: amountUSD,
          receiverAddress: address!,
          desiredToken: selectedToken,
        });
