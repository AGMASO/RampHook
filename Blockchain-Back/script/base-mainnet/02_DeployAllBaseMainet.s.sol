...
contract DeployAll is Script {
    ...
    IERC20 constant token0 = IERC20(0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913); // USDC
    IERC20 constant token1 = IERC20(0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb); // DAI

    int24 constant tickSpacing = 1;
    uint256 constant token0Amount = 5e18; // 1,000 USDC (6 decimals)
    uint256 constant token1Amount = 5e18; // 1,000 USDT (6 decimals)
    ...
    uint160 startingPrice = 79206326636808238933262; //price calculeted with extra script
    ...
    function deployHook() public {
        ...
        CustomERC20(Currency.unwrap(currency1)).transfer(
            address(vault),
            5e6 // 5 USDC
        );
    ...
    function makeOnRampOrder() public {
        ...
        Vault.OnrampData memory onRampData = Vault.OnrampData({
            ...
            desiredToken: Currency.unwrap(key.currency0) // DAI
        });
        ...
        RampHookV1.OnRampOrder[] memory pendingOrders = ramphook
            .getPendingOrders(key.toId(), false);
    ...
    function makeSwap() public {
        ...
        IERC20(Currency.unwrap(currency0)).approve(address(swapRouter), 1e18);
        ...
        console2.log(
            "DAI balance Swapper after swap:",
            IERC20(Currency.unwrap(currency0)).balanceOf(swapper)
        );
        console2.log(
            "USDC balance Swapper after swap:",
